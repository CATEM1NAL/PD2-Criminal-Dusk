...
Hooks:OverrideFunction(ShotgunBase, "_fire_raycast", function(self, user_unit, from_pos, direction, dmg_mul, shoot_player, spread_mul, autohit_mul, suppr_mul)
  ...
  do
      ...
      for _, hit in ipairs(ray_hits) do
        ...
        if not char_dmg_ext then
          ...
          if not hit.unit:in_slot(self.shield_mask) or not base_ext then
            ...
            if alert_rays then alert_rays[#alert_rays + 1] = hit end
          ...
          if alert_rays then alert_rays[alert_rays_lookup[unit_key]] = hit end
          else hit_effects[#hit_effects + 1] = hit end
        end
    ...
    mvector3.cross(mvec_right, direction, math.UP)
    mvector3.normalize(mvec_right)
    mvector3.cross(mvec_up, direction, mvec_right)
    mvector3.normalize(mvec_up)

    for i = 1, self._rays do
      mvector3.set(mvec_ax, mvec_right)
      mvector3.set(mvec_ay, mvec_up)
      mvector3.set(mvec_spread_direction, direction)

      local theta = math.random() * 360

      mvector3.multiply(mvec_ax, math.rad(math.sin(theta) * (math.random() * spread_x) * spread_mul))
      mvector3.multiply(mvec_ay, math.rad(math.cos(theta) * (math.random() * spread_y) * spread_mul))
      mvector3.add(mvec_spread_direction, mvec_ax)
      mvector3.add(mvec_spread_direction, mvec_ay)
      ...
      if can_autoaim then
        can_autoaim = false
        local weight = 0.1
        if hit_enemy then self._autohit_current = (self._autohit_current + weight) / (1 + weight)
          ...
          if auto_hit_candidate then
            ...
            if autohit_mul then autohit_chance = autohit_chance * autohit_mul end
          ...
          if hit_enemy then self._autohit_current = (self._autohit_current + weight) / (1 + weight)
          elseif auto_hit_candidate then self._autohit_current = self._autohit_current / (1 + weight) end
        end
      end
  ...
  do
    local function sort_f(a, b) return a.distance < b.distance end

    table.sort(all_hits, sort_f)
    table.sort(hit_effects, sort_f)
    if alert_rays then table.sort(alert_rays, sort_f) end
  end
  ...
  for _, hit in ipairs(hit_effects) do bullet_class:on_collision_effects(hit, self._unit, user_unit, damage) end
      ...
      if dmg > 0 then
        ...
        if hit_result then
          ...
          if hit_result.type == "death" then
            ...
            if is_civilian then kill_data.civilian_kills = kill_data.civilian_kills + 1
            else cop_kill_count = cop_kill_count + 1 end
  ...
  elseif all_enemies_hit and self._suppression then
    ...
    for u_key, enemy in pairs(all_enemies_hit) do
      if all_enemies[u_key] then result.enemies_in_cone[u_key] = { error_mul = 1, unit = enemy } end
    end
  end

  if alert_rays then result.rays = alert_rays end

  if self._autoaim then
    self._shot_fired_stats_table.hit = hit_anyone
    self._shot_fired_stats_table.hit_count = hit_count

    if not self._ammo_data or not self._ammo_data.ignore_statistic then managers.statistics:shot_fired(self._shot_fired_stats_table) end
  end
